...
object Warehouse:
  ...
  private case class WarehouseImpl() extends Warehouse:
    var store: Sequence[Item] = Sequence()
    override def store(item: Item): Unit = store = store.concat(Sequence(item))
    override def searchItems(tag: String): Sequence[Item] =
      store.filter(item => item.tags.contains(tag))
    override def retrieve(code: Int): Optional[Item] =
      store.find(item => item.code == code)
    override def remove(item: Item): Unit = store = store.filter(i => i != item)
    override def contains(itemCode: Int): Boolean = !retrieve(itemCode).isEmpty
...
@main def mainWarehouse(): Unit =
  ...
  println:
    warehouse.store(
      xiaomiMoped
    ) // side effect, add xiaomi moped to the warehouse
  println:
